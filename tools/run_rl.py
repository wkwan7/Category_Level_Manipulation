...
def parse_args():
    ...
    # If we use imitation data from experts
    parser.add_argument('--expert-from', default=None, help='the expert model for warm up')

    # Feature testing
    parser.add_argument('--feature-path', default=None, help='enable the test of the distribution of features, the path of saved file')
    parser.add_argument('--feature-only', help='expert feature distribution enabled', action='store_true', default=False)
    parser.add_argument('--policy-feature-only', help='policy generated feature distribution enabled', action='store_true', default=False)
...
def main_mfrl_brl(cfg, args, rollout, evaluator, logger):
    ...
    feature_only = False
    policy_feature_only = False
    if cfg.get('feature_only', None) is not None: feature_only = cfg.feature_only
    if cfg.get('policy_feature_only', None) is not None: policy_feature_only = cfg.policy_feature_only

    if cfg.get('feature_path', None) is not None:
        feature_savepath = cfg.feature_path
    else:
        feature_savepath = None

    print(f"feature_only:{feature_only}; policy_feature_only:{policy_feature_only}")
    if not args.evaluation:
        ...
        if cfg.get('expert_replay_cfg',None) is not None:
            is_gail = True
            if cfg.get('expert_replay_split_cfg',None) is not None:
                expert_replay = dict()
                expert_replay_split_cfg = cfg.expert_replay_split_cfg
            else:
                expert_replay = build_replay(cfg.expert_replay_cfg)
                expert_replay_split_cfg = None
            ...
            train_rl(agent, rollout, evaluator, cfg.env_cfg, replay, work_dir=cfg.work_dir, eval_cfg=cfg.eval_cfg, expert_replay=expert_replay, tmp_replay=tmp_replay, is_GAIL=is_gail,
                 feature_only=feature_only, policy_feature_only=policy_feature_only, feature_savepath=feature_savepath, expert_replay_split_cfg=expert_replay_split_cfg,
                 **cfg.train_mfrl_cfg)
        else:
            train_rl(agent, rollout, evaluator, cfg.env_cfg, replay, work_dir=cfg.work_dir, eval_cfg=cfg.eval_cfg,
                 feature_only=feature_only, policy_feature_only=policy_feature_only, feature_savepath=feature_savepath,
                 **cfg.train_mfrl_cfg)
...
def main():
    ...
    if args.feature_path is not None:cfg.feature_path = args.feature_path
    if args.feature_only is not None:cfg.feature_only = args.feature_only
    if args.policy_feature_only is not None:cfg.policy_feature_only = args.policy_feature_only
