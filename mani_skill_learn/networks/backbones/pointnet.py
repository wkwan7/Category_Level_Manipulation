...
class PointBackbone(nn.Module):
    ...
    def forward(self, pcd, feature_only = False, progressive_PN = False, progressive_TN = False, PN_alpha = 0, enable_TN_progressive = False, TN_inc_iter = 0):
        pcd = pcd.copy()
        if isinstance(pcd, dict):
            if 'pointcloud' in pcd:
                ...
                del pcd['pointcloud']
            assert 'pcd' in pcd
            return self.forward_raw(**pcd, feature_only=feature_only, 
            progressive_PN = progressive_PN, progressive_TN = progressive_TN, PN_alpha = PN_alpha, enable_TN_progressive = enable_TN_progressive, TN_inc_iter = TN_inc_iter)
        else:
            return self.forward_raw(pcd, feature_only=feature_only, 
            progressive_PN = progressive_PN, progressive_TN = progressive_TN, PN_alpha = PN_alpha, enable_TN_progressive = enable_TN_progressive, TN_inc_iter = TN_inc_iter)

    def forward_raw(self, pcd, state=None, feature_only = False, progressive_PN = False, progressive_TN = False, PN_alpha = 0, enable_TN_progressive = False, TN_inc_iter = 0):
...
class PointNetWithInstanceInfoV0(PointBackbone):
    ...
    def forward_raw(self, pcd, state, feature_only = False, progressive_PN = False, progressive_TN = False, PN_alpha = 0, enable_TN_progressive = False, TN_inc_iter = 0):
        """
        :param pcd: point cloud
                xyz: shape (l, n_points, 3)
                rgb: shape (l, n_points, 3)
        ...
        if self.attn is not None:
            ...
            if (not progressive_PN) or PN_alpha>0:
                ...
                global_feature_y = self.attn(obj_features, obj_attn_mask, progressive_TN=progressive_TN, enable_TN_progressive=enable_TN_progressive, TN_inc_iter=TN_inc_iter)  # [B, F]

            if progressive_PN:
                global_feature_x = torch.max(obj_features, 1).values
                if PN_alpha<1e-10:
                    global_feature = global_feature_x
                else:
                    global_feature = PN_alpha*global_feature_y + (1-PN_alpha)*global_feature_x
            else:
                global_feature = global_feature_y
